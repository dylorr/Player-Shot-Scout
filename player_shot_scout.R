...
player_shots <- function (specify_team, specify_year, specify_player, three_pt_only=FALSE){
  source('~/Downloads/plotcourt.R')
  library(ncaahoopR)
  vector = get_game_ids(team = specify_team, season = specify_year)
  team_shots = get_shot_locs(vector)
  attach(team_shots)
...
shooting_percentage = nrow(made) / total_shots

#summary by location
shot_area_summary = merge(made_dist_loc,missed_dist_loc, by = "Var1")
shot_area_summary["Shooting Percentage"] = shot_area_summary$Freq.x / (shot_area_summary$Freq.x + shot_area_summary$Freq.y)
shot_area_summary["Percentage of Shots"] = (shot_area_summary$Freq.x + shot_area_summary$Freq.y) / (sum(shot_area_summary$Freq.x) + sum(shot_area_summary$Freq.y))
...
shot_zone_summary["Shooting Percentage"] = shot_zone_summary$Freq.x / (shot_zone_summary$Freq.x + shot_zone_summary$Freq.y)
shot_zone_summary["Percentage of Shots"] = (shot_zone_summary$Freq.x + shot_zone_summary$Freq.y) / (sum(shot_zone_summary$Freq.x) + sum(shot_zone_summary$Freq.y))
...
end_plot <- plot_court() + geom_point(data = tomas, aes(x = x, y=y, col = outcome), alpha = 0.8) + ggtitle(paste(specify_player,' (',specify_year,') ', "tracked shots", ' (', total_shots, ')', sep="")) + theme(plot.title = element_text(hjust = 0.5, size = 16, face = 'bold'))
...
player_shots(specify_team = 'Virginia Tech', specify_year = '2019-20', specify_player = 'Wabissa Bede')
